...
"""
    ...
    def _write_inputs(self) -> None:
        ...
        manifest = {
            ...
            "sections": [{"section_id": "section-001", "blocks": blocks}],
        }
        plan = {
            "operation_id": "O-07",
            "run_id": "run-validation",
        ...
        packet = {
            ...
            "result": "ADVANCE",
            "source_catalog": [{"url": "https://official.example/spec", "final_url": "https://official.example/spec"}],
        }
    ...
    def test_validates_o12_revised_draft(self) -> None:
        manifest = json.loads(self.manifest_path.read_text(encoding="utf-8"))
        manifest.update({"operation_id": "O-12", "state_after": "REVISED", "revision_count": 1})
        self.manifest_path.write_text(json.dumps(manifest, ensure_ascii=False), encoding="utf-8")
        result = self._run()
        report = json.loads(result.report_path.read_text(encoding="utf-8"))
        self.assertEqual((result.state_after, result.result), ("VALIDATED", "ADVANCE"))
        self.assertEqual(report["state_before"], "REVISED")
    ...
    def test_unplanned_external_url_holds(self) -> None:
        ...
        manifest["sections"][0]["blocks"][0]["sha256"] = hashlib.sha256(
            "https://unknown.example の説明です。".encode()
        ).hexdigest()
        self.manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    ...
    def test_duplicate_title_holds(self) -> None:
        ...
        (self.posts_dir / "existing.rst").write_text(
            "変更を安全に運用する方法\n============================\n", encoding="utf-8"
        )
        self.assertEqual(self._run().result, "HOLD")
    ...
    def test_similarity_candidate_is_warning_not_program_error(self) -> None:
        self.posts_dir.mkdir()
        (self.posts_dir / "existing.rst").write_text(
            self._draft().replace("変更を安全に運用する方法", "別の題名です"), encoding="utf-8"
        )
        result = self._run()
        report = json.loads(result.report_path.read_text(encoding="utf-8"))
    ...
    def test_policy_candidate_holds_for_human_policy_action(self) -> None:
        ...
        judgment["policy_change_candidate"] = {
            "required": True,
            "title_ja": "恒久方針が必要です",
            "problem_ja": "現行方針では判定できません。",
            "options": [
                {"option_ja": "現状維持", "impact_ja": "公開を保留します。"},
                {"option_ja": "方針追加", "impact_ja": "今後の検査が変わります。"},
            ],
        }
        self.judgment_path.write_text(json.dumps(judgment, ensure_ascii=False), encoding="utf-8")
        result = self._run()
