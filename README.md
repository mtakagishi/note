...
このリポジトリでは、依存関係の管理・実行・検証は原則として Rye 経由で行います。

- 依存関係の同期: `rye sync`
- テスト実行: `rye run python -m unittest ...`
- ビルド: `rye run doit doc`
- 追加の Python スクリプト実行: `rye run python ...`

ローカルの直接実行や `python ...` への置き換えは、短いデバッグや確認用途に限定し、共有・再現性を重視する作業では Rye を優先します。
