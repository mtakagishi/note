...
使い方:
    ...
    rye run python export_sessions.py --latest     # 最新1件だけ（時刻付きで自動命名）
...
def _next_available_path(directory: Path, stem: str, suffix: str = ".md") -> Path:
    """同名ファイルがある場合は _001, _002... を付けて衝突を避ける。"""
...
def _build_auto_out_path(base_dir: Path, session_id: str | None, latest: bool) -> Path:
    """自動命名時の出力パスを生成する。"""
    ts_tag = datetime.now().strftime("%Y-%m-%d-%H%M%S")

    if session_id:
        stem = f"copilot-session-{session_id[:8]}-{ts_tag}"
    elif latest:
        stem = f"copilot-session-latest-{ts_tag}"
    else:
        stem = f"copilot-sessions-{ts_tag}"

    return _next_available_path(base_dir, stem)
