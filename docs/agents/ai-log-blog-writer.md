...
4. ログから次を抽出する。
   ...
   - まだ納得できていないこと
   - 次に活かせること
5. 抽出した内容を「事実」「推測」「未確認」に分けて整理する。
6. ログから読み取れる AI の結論と、ユーザー本人の納得感が一致しているか確認する。
7. 不足情報が記事品質に影響する場合は、作成前に日本語でインタビューする。
8. 検索で記事にたどり着く読者が使いそうな言葉、困りごと、調査動機を本文に自然に含める。
9. インタビューは一度に多くしすぎず、原則 1 回あたり 1 から 3 問に絞る。
10. 事実が不明な部分は推測で断定しない。
11. 読者にとっての学びが伝わる構成にする。
12. `docs/blog/posts/YYYY-MM-DD-slug.rst` を作成する。
13. 記事末尾に `.. rubric:: 記事情報` を置き、著者、公開日を明記する。
14. 必要に応じて `rye run doit doc` でビルド確認する。
15. 記事化前の確認として `docs/agents/log-extraction-checklist.md` のチェックを実施する。
