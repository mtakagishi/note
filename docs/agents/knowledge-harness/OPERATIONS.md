...
- 終了時は、Issue #2のHANDOFFコメントへ、実行ログ要約を3手順で転記する
...
### Review Packetで再確認する記事側判断観点

記事側の最終判断（O-11）に入る前に、Review Packetで次の4観点を再確認する。

1. 中心メッセージと対象読者価値
  - 何を伝える記事かが1文で読め、対象読者の再利用価値が明記されている
2. 根拠と不確実性
  - 主要根拠が出典参照付きで示され、不確実性の扱い（明示・限定・除外）が矛盾していない
3. 判断境界
  - AI担当範囲と人間判断範囲が分離され、公開承認が人間判断であることが明示されている
4. 判断選択肢と無回答時の扱い
  - 「承認 / 今回だけ修正 / 公開しない / 方針検討」の4選択肢と、無回答時は`HOLD`維持が明記されている

上記4観点のいずれかが欠ける場合は、公開判断へ進まず`HOLD`として記録する。
