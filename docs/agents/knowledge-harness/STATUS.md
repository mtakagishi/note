...
- この文書は、知識ハーネスの方法論と実装運用の進捗を記録する台帳である
- この文書の対象は方法論改善に限定し、記事1本ごとの状態遷移やHOLD理由は、`PIPELINE.md`の契約と`run_id`成果物（Outcome、Review系成果物）を正本とする
- この文書の「次の一手」は方法論改善タスクを指し、記事個別の改稿指示や公開可否判断は含めない
- 議題種別が「①優良記事作成」である場合、この文書には記事の進捗や判断を記録しない
- 議題種別が「②方法論改善」である場合だけ、この文書を更新対象とする
- 同一セッションで①と②を混在させた場合は、作業を分割して片方だけを完了または中断記録として残す
...
- 再開プロンプト半自動化の境界を`resume-minimal-checklist.md`と`OPERATIONS.md`へ反映
- 半自動化I/O契約（入力、出力、停止条件）を再開手順へ明記
...
- Review Packetで記事側判断の観点（中心メッセージ、根拠・不確実性、判断境界、4選択肢）を`OPERATIONS.md`へ運用メモ化
